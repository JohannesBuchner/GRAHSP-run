...
plt.plot(sed.wavelength_grid[m], sed.luminosities.sum(axis=0)[m], label='BC03', lw=0.3)

sed = SED()
sfh.process(sed)

m2005 = creation_modules.get_module(
    'm2005', imf=0, metallicity=0.02, separation_age=10)
m2005.process(sed)

m = np.logical_and(sed.wavelength_grid > 50, sed.wavelength_grid < 1e5)
plt.plot(sed.wavelength_grid[m], sed.luminosities.sum(axis=0)[m], label='Maraston+2005', lw=0.3)
...
plt.savefig('plotstellarpop.pdf')
plt.close()
